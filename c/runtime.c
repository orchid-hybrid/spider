...
void refcount_dec(struct scm s, int j) {
    while ((j > 0)) {
        refcount_dec_one(s);
        j = (j - 1);
    }
...
void refcount_dec_one(struct scm s) {
    int i;
    struct scm_vector* v;
    if (((2 == s.tag) || (3 == s.tag))) {
        v = s.val.v;
...
struct scm scm_vector_ref(struct scm env, struct scm vec, struct scm idx) {
    ...
    refcount_inc(ret, 1);
    refcount_dec_one(env);
    refcount_dec_one(vec);
    refcount_dec_one(idx);
    return ret;
}
...
void main() {
    refcount_dec_one(scm_main((struct scm){  }));
}
