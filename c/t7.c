...
struct scm foo(struct scm env2498, struct scm v) {
  ...
  scm_print(v,v);
    ...
    scm_vector_insert_bang(r13, v, 0);
    ...
    r7 = clo11(env12, (struct scm){ .tag = 0, .val.i = 2 }, r9);
...
struct scm g2500(struct scm env2498, struct scm i) {
  printf("[G2500 %d %d]\n", env2498.tag, i.tag);
  printf("[G2500.env %d %d]\n", env2498.val.v->len, env2498.val.v->elt[0].tag, env2498.val.v->elt[1].tag);
